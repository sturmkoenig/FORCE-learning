...
import multipop_c_13_2 as mpc #custom-made C simulation of networks and populations
#import multipop_tf as mpc_tf #custom-made C simulation of networks and populations
...
class MultiPop(object):
    ...
    def __build_network_tilo_common__(self):
        ...
        if self.impl_type == 'cc':
            mpc_impl = mpc.Multipop
        elif self.impl_type == 'tf':
            #mpc_impl = mpc_tf.Multipop
            raise NotImplemented("TF version coming soon")
        else:
            raise ArgumentError('implementation type not recognized')

        self.mp = mpc_impl(self.dt_rec, self.dt, \
        ...
        if os.path.exists(fname) and not ForceSim:
            ...
            if self.__sim_mode__==None:
                ...
                if (step!=None):
                    ...
                    if 'nest_stepcurrent' not in self.__dict__:
